...
.NOTES
    ...
    $PSVersionTable for where it was written:
        (no change in $psversiontable for 1.2 update, as of 12-7-2020)
        Name                           Value                                                                                          
        ----                           -----                                                                                          
    ...
    Change Notes:
        ...
        Description:
                    -Modified Remove-Factor.  Added support for removing custom HOTP factors using the below information
                    as a reference: https://developer.okta.com/docs/reference/api/factors/#enroll-custom-hotp-factor
                    {
                        ...
                        "status": "ACTIVE",
                    -Added a new flag, $HOTP.  Added a new variable, $hotp_fac_id.
                    -Removed a few extra semicolons in the remove-factor function.  In the "If($Google){ Try" section
                    there was an extra semicolon. 
                    -Removed a semicolon from end of line: ElseIf($line.provider -eq "GOOGLE"){$Google_fac_id = $line.id} 
                    -No change to the $PSVersionTable
                    -Added line: ElseIf($line.FactorType -eq "token:hotp"){$hotp_fac_id = $line.id}
                    This will get the hotp factor ID for a given user, if it is setup.  We need that ID to remove the factor.
        Date: 04-29-2020 - Actor: Justin Sappington
        Description:
                    -Added Remove-Factor-CSV function to remove Google factors using an input from a list of users in a csv.
        Date: 12-07-2020 - Actor: Justin Sappington
        Description:
                    -Enabling SMS and Voice switches in add-factor-csv.  Needed to split off the Google implementation
                    as it was not gated behind a switch.  Now gated behind the -Google switch
                    -cleaning up variable names in error handling.  Originally used $error which is a standard verb, switching to 
                    $error_desc
...
function Add-Factor{
    ...
    If($SMS){        
        ...
        Catch{
            $error_desc = $null
            $error_desc = Get-Error $_
            #invalid phone number error code
            If($error_desc.errorCode -eq "E0000098"){
                Return $error_desc.errorSummary + " (SMS)"
            }
            Elseif($error_desc.errorCode -eq "E0000001"){                
                Return $error_desc.errorCauses.errorSummary + " (SMS)"
            }
            Else{return $error_desc}
        }
    }
...
function Remove-Factor{
    ...
    Foreach($line in $factor_list){
        ...
        ElseIf($line.FactorType -eq "token:hotp"){$hotp_fac_id = $line.id}
    }
    ...
    If($HOTP){
        Try{Remove-OktaFactor $id $hotp_fac_id;$count++}
        Catch{
            $error = $null
            $error = Get-Error $_
            If($error.errorCode -like "*E0000022*"){Write-Output "HOTP Factor is not setup."}
            Else{Return $error.ErrorSummary}
        }    
    }
...
function Add-Factor-CSV{
    Param(
        ...
        [Parameter(mandatory=$false)]$Phone_Col="",
        [Switch]$No_Force
    )
    ...
    foreach($user in $in_csv){
        ...
        Try{
            ...
            #here it becomes factor specific
            
            #phone setups ($Voice or $SMS switches)
            # Add-Factor -Voice $email $user.$email_Col_Name -No_Connect
            If($Voice -or $SMS){

                #check to make sure phone number isn't blank. If so, jump out of loop
                If($user.$Phone_Col -eq ""){
                    $out_text = "Please provide phone number for " + $user.$Email_Col_Name + "."
                    Write-Output $out_text
                    $user.$MFA_Col = "Phone number blank, MFA not setup."
                    Continue
                }

                #if the $no_force flag is set, then we do not attempt to delete any old phone mfa
                if($No_Force -ne $true){
                    $remove_result = $null
                    If($sms -and $Voice){
                        $remove_result = Remove-Factor -Email $user.$email_Col_Name -Voice -SMS -No_Connect
                    }
                    ElseIf($SMS){
                        $remove_result = Remove-Factor -Email $user.$email_Col_Name -SMS -No_Connect
                    }
                    Else{
                        $remove_result = Remove-Factor -Email $user.$email_Col_Name -Voice -No_Connect
                    }
                ...
                #voice and SMS
                If($voice -and $SMS){
                    $Fac_setup_result = Add-Factor -Voice -SMS -Email $user.$email_Col_Name -No_Connect -Phone $user.$Phone_Col
                }
                #just Voice
                ElseIf($Voice){
                    $Fac_setup_result = Add-Factor -Voice -Email $user.$email_Col_Name -No_Connect -Phone $user.$Phone_Col
                }
                #just SMS
                Else{
                    $Fac_setup_result = Add-Factor -SMS -Email $user.$email_Col_Name -No_Connect -Phone $user.$Phone_Col
                }

                if($Fac_setup_result -like "*A factor of this type is already set up.*"){
                    $user.$MFA_Col = $Fac_setup_result[0]#"A factor of this type is already set up."
                }
                Elseif($Fac_setup_result -eq "User not found."){
                    $user.$MFA_Col = "User not found."
                }
                else{
                    $user.$MFA_Col = $Fac_setup_result
                }
...
function Remove-Factor-CSV{
    Param(
        ...
        [Parameter(mandatory=$false)]$FirstName_Col="",
        [Parameter(mandatory=$false)]$LastName_Col="",
        [Parameter(mandatory=$false)]$Result_Col_name=""
    )
    ...
    #check for a result column, create  it if it doesn't exist
    if($Result_Col_name -eq ""){
        $column_names = $null
        $column_names = (Get-Member -InputObject $in_csv[0]).name
        Foreach($object in $column_names){
            If($object -like "Result"){
                $Result_Col_name = $object
                Break
            }
        }
    }

    #if we did not find a result column, then we create it.
    if($Result_Col_name -eq ""){
        $Result_Col_name  = "Result"
        $in_csv = $in_csv | Select-Object *, @{name = $Result_Col_name;expr={}}
    }
    ...
    foreach($user in $in_csv){
        ...
        Try{
            ...
            #perform the factor removal
            $remove_result = $null
            $remove_result = Remove-Factor -Email $user.$email_Col_Name -Google -No_Connect
            If($remove_result -like "Removed 1 factor for user *"){
                $user.$Result_Col_name = $remove_result
            }
            #if removal fails, it comes back as an object so we need the first object for our output
            Else{$user.$Result_Col_name = $remove_result[0]}
        }

        Catch{
            If($_ -like "*(404) Not Found*"){$user.$Result_Col_name = "User not found."}
            Else{$_}
        }
        $out_text = $null
        $out_text = ($user.$email_Col_Name)+" - "+($user.$Result_Col_name)
        Write-Output $out_text
    }
    ...
    $dtg = Get-Date -Format "MM-dd-yyyy_HHmmss"
    #create file name
    $new_csv_file_name = $new_csv_path+"MFA Removal Results_"+$dtg+".csv"
    $in_csv | Export-Csv -Path $new_csv_file_name -NoTypeInformation -Force
}
#endregion